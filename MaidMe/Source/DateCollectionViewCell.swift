...
class DateCollectionViewCell: UICollectionViewCell {
    ...
    override var isSelected: Bool {
        didSet {
            dayLabel.textColor = isSelected == true ? UIColor.white : darkColor.withAlphaComponent(0.5)
            numberLabel.textColor = isSelected == true ? UIColor.white : darkColor
            contentView.backgroundColor = isSelected == true ? highlightColor : UIColor.white
            contentView.layer.borderWidth = isSelected == true ? 0 : 1
        }
    }
    
    func populateItem(_ date: Date, highlightColor: UIColor, darkColor: UIColor) {
		...
		if Calendar.current.isDateInToday(date) {
			dayLabel.text = "Today".uppercased()
		}else if Calendar.current.isDateInTomorrow(date) {
			dayLabel.text = "Tomorrow".uppercased()
		}else{
